...
from can_fuzzing.fuzzing.obd_fuzz import decode_response, encode_request_frames, summarize_responses


class OBDFuzzProtocolHelperTests(unittest.TestCase):
    def test_encode_request_frames_uses_isotp_single_frame(self) -> None:
        frames = encode_request_frames(bytes([0x01, 0x0C]))
        self.assertEqual(frames, [bytes([0x02, 0x01, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00])])

    def test_decode_positive_pid_response(self) -> None:
        frame = decode_response(bytes([0x04, 0x41, 0x0C, 0xAA, 0xBB, 0x00, 0x00, 0x00]), 0x01)
    ...
    def test_decode_negative_response(self) -> None:
        frame = decode_response(bytes([0x03, 0x7F, 0x01, 0x12, 0x00, 0x00, 0x00, 0x00]), 0x01)
